...
Options:
    ...
    -d --image-dir DIR  Image directory [default: ./data/dailymail/images].
    -f --face-dir DIR   Image directory [default: ./data/dailymail/facenet].
...
def detect_faces(article, dailymail, image_dir, face_dir, mtcnn, resnet):
    ...
    try:
        dailymail.articles.find_one_and_update(
...
def main():
    ...
    article_cursor = dailymail.articles.find(no_cursor_timeout=True).batch_size(128)
    ...
    for article in tqdm(article_cursor):
        detect_faces(article, dailymail, image_dir, face_dir, mtcnn, resnet)

    # with Parallel(n_jobs=8, backend='threading') as parallel:
    #     parallel(delayed(detect_faces)(article, dailymail, image_dir, face_dir, mtcnn, resnet)
    #              for article in tqdm(article_cursor))

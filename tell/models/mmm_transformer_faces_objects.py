...
from tell.utils import beam_search, beam_search_fake, beam_search_fake_coh2
...
@Model.register("mmm_transformer_faces_objects")
class MMMTransformerFacesObjectModel(Model):
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 criterion: Criterion,
    ...
    def forward(self,  # type: ignore
                image_count: Dict[str, torch.LongTensor],
                article_index: Dict[str, torch.LongTensor],
                image_index: Dict[str, torch.LongTensor],
                context: Dict[str, torch.LongTensor],
                image: torch.Tensor,
                caption: Dict[str, torch.LongTensor],
                fake_captions: Dict[str, torch.LongTensor],
        ...
        #image_pos = [m['image_pos'] for m in metadata]
        #print("article_ids", article_ids)
        #print("image_pos", image_pos)
        caption_ids, target_ids, fake_captions_ids, contexts = self._forward(
            context, image, caption, fake_captions, face_embeds, obj_embeds)
        if self.lambda1 == 0 and self.lambda2 == 0:
            decoder_out = self.decoder(caption, contexts, None) 
        else:
            decoder_out = self.decoder(caption, contexts, fake_captions) 

        # Assume we're using adaptive loss
        loss0, sample_size, fake_loss, fake_size, coh_loss, coh_size, mm_loss, mm_size, coh_score = self.criterion(
            self.decoder.adaptive_softmax, decoder_out, target_ids, caption_ids, fake_captions_ids, article_index)
        
        '''
        loss = loss / math.log(2) / sample_size
        fake_loss = self.lambda1 * fake_loss / math.log(2) / sample_size
        coh_loss = self.lambda2 * coh_loss / math.log(2) / sample_size
        mm_loss = self.lambda3 * mm_loss / math.log(2) / sample_size
        '''
        '''
        loss = loss / math.log(2)
        if self.lambda1 > 0:
            loss += self.lambda1 * fake_loss / math.log(2)
        if self.lambda2 > 0:
            loss += self.lambda2 * coh_loss / math.log(2)
        if self.lambda3 > 0:
            loss += self.lambda3 * mm_loss / math.log(2)
        '''
        
        loss = 0
        if self.lambda0 > 0:
            loss = self.lambda0 * loss0 / math.log(2) / sample_size
        if self.lambda1 > 0:
            loss += self.lambda1 * fake_loss / math.log(2) / fake_size
        if self.lambda2 > 0:
            loss += self.lambda2 * coh_loss / math.log(2) / coh_size
        if self.lambda3 > 0:
            loss += self.lambda3 * mm_loss / math.log(2) / mm_size
        
        output_dict = {
            "loss": loss,
            #"loss": [coh_loss, fake_loss, mm_loss, loss],
        ...
        if not self.training and self.evaluate_mode:
            if self.gen_coh == "coh":
                output_dict["coh"] = coh_score.cpu().detach().numpy() 
            elif self.gen_coh == "gen":
                if self.dec_method == "greedy":
                    _, gen_ids, attns = self._generate(caption_ids, contexts, attn_idx)
                elif self.dec_method == "beam-search":
                    _, gen_ids, attns = self._generate_beam_search(caption_ids, contexts, article_index, attn_idx)
    ...
    def _forward(self,  # type: ignore
        ...
        target_ids = torch.zeros_like(caption_ids)
        target_ids.fill_(self.padding_idx)
        target_ids[:, :-1] = caption_ids[:, 1:]
        ...
        fake_captions_ids = fake_captions[self.index]
        ...
        return caption_ids, target_ids, fake_captions_ids, contexts
    ...
    def _generate_beam_search(self, caption_ids, contexts, article_index, attn_idx=None, type="fake-coh"):
        '''
        ...
        '''
        if type == "normal":
            log_probs, token_ids, attns = beam_search.beam_search_generate(self.decoder, 
                                                                           self.index,
                                                                           caption_ids, 
                                                                           contexts, 
                                                                           caption_ids.size(0), 
            ...
            log_probs, token_ids, attns = beam_search_fake.beam_search_generate(self.decoder, 
                                                                           self.index,
                                                                           self.criterion.fake_score_sigmoid, 
                                                                           caption_ids, 
                                                                           contexts, 
                                                                           caption_ids.size(0), 
                                                                           num_beams=self.beam_size)
            ...
            decoder,
                         ...
                         contexts,
            '''
            log_probs, token_ids, attns = beam_search_fake_coh2.beam_search_generate(self.decoder, 
                                                                                    self.index,
                                                                                    self.criterion.fake_score_sigmoid, 
                                                                                    self.criterion.coh_score_sigmoid, 
                                                                                    caption_ids, 
                                                                                    article_index, 
                                                                                    contexts, 
                                                                                    num_beams=self.beam_size)

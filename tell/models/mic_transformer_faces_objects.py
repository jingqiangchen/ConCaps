...
from tell.utils import beam_search, beam_search_fake

from .decoder_flattened import Decoder
from .resnet import resnet152


@Model.register("mic_transformer_faces_objects")
class MICTransformerFacesObjectModel(Model):
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 criterion: Criterion,
    ...
    def forward(self,  # type: ignore
        ...
        decoder_out = self.decoder(caption, fake_captions, contexts) 

        # Assume we're using adaptive loss
        loss, sample_size, fake_loss, batch_size = self.criterion(
            self.decoder.adaptive_softmax, decoder_out, target_ids, caption_ids, fake_captions_ids)

        loss = loss / math.log(2)
        fake_loss = fake_loss / math.log(2)

        output_dict = {
            'loss': loss / sample_size + self.lambda1 * fake_loss / batch_size,
    ...
    def _generate_beam_search(self, caption_ids, contexts, attn_idx=None, type="normal"):

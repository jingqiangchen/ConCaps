...
@Decoder.register('mmm_dynamic_conv_decoder_faces_objects')
class MMMDynamicConvFacesObjectsDecoder(Decoder):
    def __init__(self, vocab, embedder: TextFieldEmbedder, max_target_positions, dropout,
        ...
        self.layers.extend([
            MMMDynamicConvDecoderLayer(embed_dim, decoder_conv_dim, decoder_glu,
                                    decoder_conv_type, weight_softmax, decoder_attention_heads,
    ...
    def forward(self, prev_target, contexts, fake_captions=None, incremental_state=None,
        ...
        #############################################
        if fake_captions is not None:
            index, fake_captions_ids = list(fake_captions.items())[0]
            btz, fakes, wno = fake_captions_ids.shape
            fake_captions[index] = fake_captions_ids.view(btz * fakes, wno)
            Y = self.embedder(fake_captions, incremental_state=incremental_state)
            Y = Y.view(btz, fakes, wno, -1).contiguous()
            
            if self.project_in_dim is not None:
                Y = self.project_in_dim(Y)
    
            Y = F.dropout(Y, p=self.dropout, training=self.training)
            
            ys = []
            for k in range(fakes):
                y = Y.narrow(1, k, 1).squeeze(1)
                # B x T x C -> T x B x C
                y = y.transpose(0, 1)
                ...
                if self.adaptive_softmax is None:
                    ...
                    if self.share_input_output_embed:
                        y = F.linear(
                            y, self.embedder.token_embedder_bert.word_embeddings.weight)
                    else:
                        y = F.linear(y, self.embed_out)
                        
                ys.append(y)
                
            Y = torch.stack(ys, dim=1)

        return X, {'attn': attns, 'inner_states': inner_states}, Y 
...
@DecoderLayer.register('mmm_dynamic_conv_faces_objects')
class MMMDynamicConvDecoderLayer(DecoderLayer):

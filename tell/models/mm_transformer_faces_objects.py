...
from tell.utils import beam_search

from .decoder_flattened import Decoder
from .resnet import resnet152


@Model.register("mm_transformer_faces_objects")
class MMTransformerFacesObjectModel(Model):
    def __init__(self,
                 vocab: Vocabulary,
                 decoder: Decoder,
                 criterion: Criterion,
    ...
    def forward(self,  # type: ignore
        ...
        decoder_out = self.decoder(caption, None, contexts) 

        # Assume we're using adaptive loss
        loss, sample_size, mm_loss, mm_size = self.criterion(
            self.decoder.adaptive_softmax, decoder_out, target_ids, caption_ids, article_index)

        loss = loss / math.log(2)
        mm_loss = mm_loss / math.log(2)
        output_dict = {
            'loss': (loss + self.lambda3 * mm_loss) / sample_size,
            'sample_size': sample_size,
            'mm_size': mm_size
        }
        #print("self.lambda3", self.lambda3)
        '''
        output_dict = {
            'loss': loss / sample_size + self.lambda3 * mm_loss / mm_size,
            'sample_size': sample_size,
            'mm_size': mm_size
        }
        '''
        ...
        if not self.training and self.evaluate_mode:
            if self.dec_method == "greedy":
                ...
                _, gen_ids, attns = self._generate_beam_search(caption_ids, contexts, attn_idx)
    ...
    def _generate_beam_search(self, caption_ids, contexts, attn_idx=None):
        a, b, c = contexts['image'].shape
        contexts_image = contexts['image'].unsqueeze(2).expand(a, b, self.beam_size, c).contiguous().reshape(a, b * self.beam_size, c)
        ...
        contexts_article = contexts['article'].unsqueeze(2).expand(a, b, self.beam_size, c).contiguous().reshape(a, b * self.beam_size, c)
        ...
        contexts_faces = contexts['faces'].unsqueeze(2).expand(a, b, self.beam_size, c).contiguous().reshape(a, b * self.beam_size, c)
        ...
        contexts_obj = contexts['obj'].unsqueeze(2).expand(a, b, self.beam_size, c).contiguous().reshape(a, b * self.beam_size, c)

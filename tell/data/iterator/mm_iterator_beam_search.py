...
def split_and_sort_by_padding(instances: List[Instance],
                    ...
                    batch_size:int = 15,
                    bs_batch_size: int = 5) -> List[Instance]:
    ...
    old_list = instances_with_lengths
    new_list = []
    old_key = old_list[0][0][0]
    i = 0
    for keys, item in old_list: 
        if i == 0 or i % batch_size != 0:
            if keys[0] == old_key and i % batch_size < bs_batch_size:
                new_list.append(item)
                i += 1
            else:
                for _ in range(i % batch_size, batch_size + 1):
                    new_list.append(item)
                    i += 1
                old_key = keys[0]
        else:
...
@DataIterator.register("mm_iterator_beam_search") 
class MMIteratorBeamSearch(DataIterator):

...
@DatasetReader.register('mm_dailymail_faces_ner_matched')
class MMDailyMailFacesNERMatchedReader(DatasetReader):
    """Read from the Daily Mail dataset.
        ...
        for article_index, article_id in enumerate(ids):
            ...
            for pos in image_positions:
                ...
                yield self.article_to_instance(
                    image_count, article_index, image_index, paragraphs, named_entities, image, caption, image_path,
                    "" if 'web_url' not in article else article['web_url'], pos, face_embeds, obj_feats)

    def article_to_instance(self, image_count, article_index, image_index, paragraphs, named_entities, image, caption, 

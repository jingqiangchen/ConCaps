...
def evaluate(model: Model,
    ...
    if 'goodnews' in serialization_dir:
        cache_path = '/home/test/mic/data/goodnews/evaluation_cache.pkl'
    elif 'nytimes' in serialization_dir:
        cache_path = '/home/test/mic/data/nytimes/evaluation_cache.pkl'
    elif 'dailymail' in serialization_dir:
        cache_path = '/home/test/mic/data/dailymail/evaluation_cache.pkl'
...
def write_to_json(output_dict, serialization_dir, nlp, eval_suffix, cache):
    if 'captions' not in output_dict:
        out_path = os.path.join(
            serialization_dir, f'generations{eval_suffix}_coh.jsonl')
...
def get_readability_scores(text):
    scores = {
        ...
        'difficult_words': textstat.difficult_words(text) / (len(text.split()) if len(text.split()) > 0 else 1),
    }
    return scores

...
@Criterion.register('mmm_adaptive_loss_coh')
class MMMAdaptiveLossCoh(Criterion):
    ...
    def forward_sigmoid(self, adaptive_softmax, net_output, decoder_target, caption_ids, fake_captions_ids, article_indices, reduction='sum'):
        ...
        if self.proj_fake is not None:
            ...
            first_article_index = article_indices.narrow(0, 0, 1) 
            first_unmask = article_indices == first_article_index
            
            mm_logits2 = self.proj_fake(X)
            # mm_logits2 = fake_logits[first_unmask.squeeze(-1)]
            coh_score = torch.mean(F.sigmoid(mm_logits2))
        ...
        if self.proj_coh is not None:
            ...
            c = torch.triu(coh_unmasks, diagonal=-1).transpose(0, 1)
            coh_unmasks = torch.logical_and(b, c)
            ...
            coh_logits1_unmasks = torch.triu(coh_unmasks,diagonal=1)
            coh_logits = coh_logits[coh_logits1_unmasks]
            ...
            for i in range(btz):
                ...
                coh_logits.append(coh_logit)
            coh_logits = torch.cat(coh_logits, 1)
            coh_logits_score = coh_logits
            coh_logits2_unmasks = torch.logical_or(coh_logits1_unmasks, coh_logits1_unmasks.transpose(0, 1))
            coh_logits = coh_logits[coh_logits2_unmasks]
            ...
            first_article_index = article_indices.narrow(0, 0, 1) 
            first_unmask = article_indices == first_article_index
            first_unmask = first_unmask.half()
            first_unmask = torch.matmul(first_unmask, first_unmask.transpose(0, 1)).bool() 
            first_unmask = torch.triu(first_unmask, diagonal=1)
            
            coh_logits3 = coh_logits_score[first_unmask.squeeze(-1)]
            coh_score = torch.mean(F.sigmoid(coh_logits3))
        ...
        if self.proj_mm is not None:
            ...
            mm_logits1 = torch.cat(mm_logits, 1)
            mm_unmask = torch.triu(mm_logits1.new(torch.ones(mm_logits1.shape).cuda()).bool(),diagonal=1)
            mm_logits = mm_logits1[mm_unmask]
            ...
            mm_logits2 = mm_logits1[first_unmask.squeeze(-1)]
            coh_score = torch.mean(F.sigmoid(mm_logits2))
            #print("-" * 100)
            #print(mm_logits2, coh_score)
            #print(article_indices) 
            #print(first_article_index)
    ...
    def coh_score_sigmoid(self, X):
        coh_logits = X
        coh_logits = self.proj_mm(coh_logits).squeeze(1) 
        coh_scores = F.logsigmoid(coh_logits)

...
def beam_search_generate(decoder,
    ...
    while cur_len < max_length:
        ...
        output = decoder(
                {data_index: torch.cat((input_ids, append_eos_ids), dim=1)},
                None,
                contexts,
        ...
        #fake_score
        future_fake_scores = fake_score_function(output[0][:, -1])
            ...
            for beam_token_rank, (beam_token_id, beam_token_score, beam_future_fake_score) in enumerate(
                ...
                if (eos_token_id is not None) and (token_id.item() == eos_token_id):
                    ...
                    generated_hyps[batch_idx].add(
                        input_ids[effective_beam_id].clone(), beam_token_score.item() + beam_future_fake_score.item(), 
                    )
                else:

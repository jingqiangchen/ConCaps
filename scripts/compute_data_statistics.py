...
def compute_nytimes_stats(nytimes):
    ...
    for article in tqdm(cursor):
        ...
        for s in sections:
            if s['type'] == 'caption':
                image_path = os.path.join('/home/test/mic/data/nytimes/images_processed',
                                          f"{s['hash']}.jpg")
...
def compute_goodnews_stats(goodnews):
    ...
    for sample in tqdm(cursor):
        ...
        image_path = os.path.join(
            '/home/test/mic/data/goodnews/images_processed', f"{sample['_id']}.jpg")
        if not os.path.exists(image_path):
            continue
...
def compute_rare_stats(nytimes):
    ...
    path = '/home/test/mic/data/nytimes/name_counters.pkl'
...
def compute_dailymail_stats(dailymail):
    projection = ['_id', 'parsed_section.type', 'pub_date', 'split',
                  'parsed_section.parts_of_speech',
                  'parsed_section.named_entities',
                  'parsed_section.hash', 'parsed_section.text']
    cursor = dailymail.articles.find(
        {'split': {'$in': ['train', 'valid', 'test']}, 'n_images': {"$lte": 10, "$gte": 1}},
        projection=projection)
    ...
    logger.info('Computing DailyMail statistics.')
    for article in tqdm(cursor):
        sections = article['parsed_section']
        pars = [s['text'] for s in sections if s['type'] == 'paragraph']
        captions = []
        ...
        for s in sections:
            if s['type'] == 'caption':
                image_path = os.path.join('/home/test//mic/data/dailymail/images_processed',
                                          f"{s['hash']}.jpg")
                if not os.path.exists(image_path):
                    continue
    ...
    print()
    '''
    Full DailyMail Dataset:
    No of articles: 182623
    No of captions: 813313
    Average article len: 555.4928240144998
    Average caption len: 22.6907906304215
    Words: 16347327
    Nouns: 3008915 (18.41%)
    Verbs: 2451466 (15.00%)
    Adjectives: 888769 (5.44%)
    Pronouns: 340109 (2.08%)
    Proper nouns: 2298577 (14.06%)
    Entity words: 2882660 (17.63%)
    Entity sents: 567158 (69.73%)
    Person names: 1013012 (6.20%)
    Person sents: 384496 (47.28%)
    Caption splits: defaultdict(<class 'int'>, {'train': 724254, 'valid': 47988, 'test': 41071})
    Article splits: defaultdict(<class 'int'>, {'train': 163615, 'valid': 10221, 'test': 8787})
    '''
...
def main():
    ...
    #compute_dailymail_stats(dailymail)
